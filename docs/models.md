...
| **Fuentes de datos en tiempo real** | (1) `POST /realtime/reading` o `RABBITMQ_QUEUE`, con el `AnomalyRequestDTO` ya armado por el backend (`ProcessRealtimeReading`). (2) **`mqtt.sensor.data.queue`** (`MqttSensorConsumer` → `ProcessMqttSensorReading`): consume lecturas crudas de UN sensor por mensaje directo del bridge MQTT, reconstruye el snapshot combinando el último valor conocido de cada sensor por circuito (`CircuitSensorStateRepository`, en memoria), y arma la misma ventana de 2h antes de invocar el modelo — es la vía pensada para que la detección reaccione a cada lectura real sin depender de que el backend ensamble nada. Ver [docs/data-flow.md](data-flow.md#31-detección-de-anomalías-dedicada--datos-crudos-de-sensores-mqttsensordataqueue). |
